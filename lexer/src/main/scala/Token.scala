...
// type is a keyword so I used _tokenType
final case class Token private (private val _tokenType: Token.Type, private val _data: Option[String]) {

	// Getters
	def tokenType = _tokenType
	def data = _data

	/* Determines if a token is equivalent to a passed in token, by checking
	 * equality of pattern, hasData, the type itself, and the ancillary data */
	def equals(that: Token): Boolean = {
		if (that == null) {
			false
		}

		else {
			val tokenEqualsBool = this.tokenType.equals(that.tokenType)
	...
	private def optionEquals(that: Token): Boolean = this.data match {
		case Some(n) => that.data match {
			case Some(x) => n == x
			case _ => false
		}
		case _ => false
	...
	// Calculates a hashcode based on the Token's parameters
	override def hashCode: Int = {
		val prime = 17
		val tokenHashCode = tokenType.hashCode
		val patternCode = tokenType.pattern.hashCode
		val ancillaryCode = data match {
	...
	/* Returns a formatted String by first calling the toString for the token type.
	 * Then, if there is ancillary data, format and append that to what we have.
	 * Return this potentially concatentated string. */
	override def toString: String = {
		val builder = new StringBuilder
		builder.append(tokenType.toString)
		builder.append(data match {
			case Some(n) => "[\"" + n + "\"]"
			case _ => "[\"" + "\"]"
		}
		)
